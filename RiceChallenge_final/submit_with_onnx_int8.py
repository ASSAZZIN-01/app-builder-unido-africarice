...
class Config:
    ...
    ONNX_MODEL = os.path.join(SCRIPT_DIR, "ultimate_tiled_multitask_int8.onnx")
...
def create_session() -> ort.InferenceSession:
    """Create ONNX Runtime session forcing CPU for INT8."""
    return ort.InferenceSession(Config.ONNX_MODEL, providers=["CPUExecutionProvider"])
...
def main() -> int:
    ...
    out_df = out_df[cols]
    out_df.to_csv("submission_onnx_int8.csv", index=False)
    print("Submission saved to submission_onnx_int8.csv")
    return 0
