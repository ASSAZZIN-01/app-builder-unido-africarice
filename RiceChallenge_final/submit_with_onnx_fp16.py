...
def main() -> int:
    ...
    for _, row in tqdm(test_df.iterrows(), total=len(test_df)):
        ...
        tile_tensors = np.expand_dims(tile_tensors, axis=0).astype(np.float16)

        meta = build_meta(row.get("Comment", "")).astype(np.float16)
        ...
        for i, col in enumerate(Config.COUNT_COLS):
            ...
            if not np.isfinite(val):
                val = 0.0
            res_row[col] = max(0, int(round(val)))
        for i, col in enumerate(Config.MEASURE_COLS):
            val = float(measures[i])
            if not np.isfinite(val):
                val = 0.0
            res_row[col] = val
    ...
    out_df = out_df[cols]
    out_df.to_csv("submission_onnx_fp16.csv", index=False)
    print("Submission saved to submission_onnx_fp16.csv")
    return 0
